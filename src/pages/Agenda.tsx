...
import { Plus, Edit2, Trash2, ChevronLeft, ChevronRight, X, Clock } from 'lucide-react';
import { getAppointments, saveAppointments, getToday, formatDateBr, getInitials, getServices } from '../utils/storage';
...
export default function Agenda() {
  ...
  useEffect(() => {
    ...
    if (loadedServices.length > 0) {
      setFormData(prev => ({ ...prev, service: loadedServices[0].title, value: loadedServices[0].price }));
    }
  }, []);
  ...
  const handleStatusChange = (id: number, newStatus: string) => {
    const newRecords = records.map(r => r.id === id ? { ...r, status: newStatus } : r);
  ...
  const handleOpenModal = (record?: any) => {
    if (record) {
      ...
      setFormData({
        ...
        client: '',
        vehicle: '',
        service: servicesList.length > 0 ? servicesList[0].title : '',
        value: servicesList.length > 0 ? servicesList[0].price : '',
        status: 'Agendado'
      });
    }
    setIsModalOpen(true);
  ...
  // Filter records: ONLY show non-finalized AND non-canceled
  const activeRecords = records.filter(r => r.status !== 'Serviço Finalizado' && r.status !== 'Cancelado');
  
  const filteredRecords = activeRecords.filter(record => {
    if (filterStatus !== 'Todos os Status' && record.status !== filterStatus) return false;
    return true;
  });
  ...
  const sortedRecords = [...filteredRecords].sort((a, b) => {
    ...
    return dateA.getTime() - dateB.getTime(); // Ascending for agenda
  });
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-white mb-1">Agenda de Serviços</h1>
          <p className="text-slate-400">Gerencie os agendamentos e serviços em andamento</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 bg-[#1e293b]/30 p-4 rounded-xl border border-[#1e293b]">
        <div className="space-y-1.5">
          <label className="text-xs font-bold uppercase tracking-wider text-slate-500">Status do Serviço</label>
          <select 
            value={filterStatus}
                ...
                <tr key={record.id} className="hover:bg-[#1e293b]/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <select
                      ...
                      className={`text-xs font-bold rounded-full px-3 py-1 outline-none cursor-pointer border appearance-none text-center ${
                        record.status === 'Em Andamento'
                          ? 'bg-amber-500/10 text-amber-500 border-amber-500/20'
                          : 'bg-[#0d93f2]/10 text-[#0d93f2] border-[#0d93f2]/20'
                      }`}
                    >
                      <option value="Agendado" className="bg-[#1e293b] text-white">Agendado</option>
                      <option value="Em Andamento" className="bg-[#1e293b] text-white">Em Andamento</option>
                      <option value="Cancelado" className="bg-[#1e293b] text-red-500 font-bold">✗ Cancelar Serviço</option>
                      <option value="Serviço Finalizado" className="bg-[#1e293b] text-emerald-500 font-bold">✓ Finalizar Serviço</option>
                    </select>
                  </td>
          ...
          <div className="bg-[#101b22] border border-[#1e293b] rounded-xl w-full max-w-md overflow-hidden shadow-2xl">
            <div className="flex justify-between items-center p-4 border-b border-[#1e293b]">
              <h3 className="text-lg font-bold text-white">
                {editingRecord ? 'Editar Agendamento' : 'Novo Agendamento'}
              </h3>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#101b22] border border-[#1e293b] rounded-xl w-full max-w-sm overflow-hidden shadow-2xl p-6">
            ...
            <p className="text-slate-400 mb-6">Tem certeza que deseja excluir este agendamento? Esta ação não pode ser desfeita.</p>
